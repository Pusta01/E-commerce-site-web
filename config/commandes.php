...
function afficherUnProduit($id)
{
  ...
  function ajouter_panier($image, $nom, $prix, $desc , $quantité_commander, $user_id)
  {
    if(require("connexion.php"))
    {
      $req = $access->prepare("INSERT INTO panier (image, nom, prix, description,quantité_commander,user_id) VALUES (?,?, ?, ?, ?, ?)");

      $req->execute(array($image, $nom, $prix, $desc, $quantité_commander, $user_id));
...
function afficher_panier()
{
	if(require("connexion.php"))
	{
		$req=$access->prepare("SELECT * FROM panier ORDER BY user_id DESC");
